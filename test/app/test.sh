...
function kvstore_over_socket(){
    ...
    pid_kvstore=$!
    tendermint node --mode validator --consensus.double_sign_check_height 0 > tendermint.log &
    pid_tendermint=$!
    sleep 5
...
function counter_over_grpc() {
    ...
    pid_counter=$!
    tendermint node --mode validator --consensus.double_sign_check_height 0 --abci grpc > tendermint.log &
    pid_tendermint=$!
    sleep 5
...
function counter_over_grpc_grpc() {
    ...
    sleep 1
    GRPC_PORT=36656
    tendermint node --mode validator --consensus.double_sign_check_height 0 --abci grpc --rpc.grpc_laddr tcp://localhost:$GRPC_PORT > tendermint.log &
    pid_tendermint=$!
    sleep 5
