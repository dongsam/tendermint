...
bash test/p2p/peer.sh "$DOCKER_IMAGE" "$NETWORK_NAME" $IPV "$ID" "$PROXY_APP" "--p2p.pex --rpc.unsafe --mode validator --consensus.double_sign_check_height 0"
...
#	bash test/p2p/peer.sh "$DOCKER_IMAGE" "$NETWORK_NAME" $IPV "$i" "$PROXY_APP" "--p2p.pex --rpc.unsafe --mode validator --p2p.persistent_peers '' --p2p.seeds $SEED_ADDR --consensus.double_sign_check_height 0"
	bash test/p2p/peer.sh "$DOCKER_IMAGE" "$NETWORK_NAME" $IPV "$i" "$PROXY_APP" "--p2p.pex --rpc.unsafe --mode validator --p2p.seeds $SEED_ADDR --consensus.double_sign_check_height 0"
	echo "started local_testnet_$i"
done

#docker rm "local_testnet_3:/go/src/github.com/tendermint/tendermint/test/p2p/data/mach2/config/addrbook.json"
#docker rm "local_testnet_4:/go/src/github.com/tendermint/tendermint/test/p2p/data/mach3/config/addrbook.json"
...
# NOTE that we do not provide persistent_peers
bash test/p2p/peer.sh "$DOCKER_IMAGE" "$NETWORK_NAME" $IPV "$ID" "$PROXY_APP" "--p2p.pex --rpc.unsafe --mode seednode --p2p.seed_mode --p2p.persistent_peers $PERSISTENT_PEERS --consensus.double_sign_check_height 0"
echo "started local_testnet_$ID"
